...
class VoxelGrid(open3d.geometry.VoxelGrid):
    def __init__(self,points,bounds,create_from_bounds=False):
        ...
        if not create_from_bounds:
            self.voxel_grid = self.create_from_point_cloud(cloud, voxel_size=0.0015)
        else:
            self.voxel_grid = self.create_from_point_cloud_within_bounds(cloud,min_bound=self.min_bounds[0:3],
                                        max_bound=self.max_bounds[0:3],voxel_size=0.0015)
        self.grid_indices = []
